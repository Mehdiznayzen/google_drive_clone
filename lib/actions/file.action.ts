...
export const renameFile = async ({ fileId, extension, path, name }: RenameFileProps) => {
    const { databases } = await createAdminClient()
    try {
...
export const handleDeleteFile = async ({ bucketFileId, fileId, path }: DeleteFileProps) => {
    ...
    try {
        ...
        handleError(error, "Failed to delete that file !!")
    }
}
